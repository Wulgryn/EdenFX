...
namespace PandoraEX::Base2D
{
    ...
    Struct(Base2D)
    {
        ...
        /// @return T The object.
        Type &set(double &val1, double &val2);

        template <class U> U to()
        {
            if (!std::is_base_of<Base2D<U>, U>::value) ThrowExceptionF(Exceptions::InvalidArgumentException, "Invalid type conversion from " + PandoraEX::Utils::demangle(typeid(*this).name()) + " to " + PandoraEX::Utils::demangle(typeid(U).name()));
            return U(val1, val2);
        }
        ...
        /// @return T The object.
        template <class U>
        Type &operator=(const Base2D<U> &b2){
            val1 = b2.val1;
            val2 = b2.val2;
            return *this;
        }
